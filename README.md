...
Redis의 일반 List(`LPUSH`/`BRPOP`)는 워커가 메시지를 꺼낸 직후 죽으면 그 메시지가 사라집니다. Streams는 **Consumer Group과 ACK 메커니즘**을 제공하므로, 워커는 MongoDB 적재가 성공한 뒤에만 `XACK`을 보냅니다. ACK 전에 워커가 죽으면 해당 메시지는 PEL에 남아 있고, 워커가 재기동하면 미처리분부터 회수해 재처리합니다.
...
<img width="1093" height="665" alt="Image" src="https://github.com/user-attachments/assets/e71c4f5c-489f-46b4-8127-5091292c0483" />
...
## 6. 세부 설계 결정 (Design Notes) - 이 내용들이 필요할까?

**`python:3.12-slim` (alpine이 아닌 이유)** — alpine은 musl libc 기반이라 일부 Python 휠 호환성 문제와 재컴파일 비용이 발생할 수 있습니다. slim은 이미지 크기와 안정성의 균형점입니다.

**멀티스테이지 빌드 + non-root** — builder 스테이지에서 의존성만 설치해 복사하므로 pip 캐시와 빌드 도구가 최종 이미지에 남지 않습니다. 두 컨테이너 모두 전용 시스템 유저(uid 10001/10002)로 실행해 컨테이너 탈취 시 피해 범위를 최소화합니다.

**healthcheck 기반 기동 순서 & 복구의 오케스트레이터 위임** — API의 `/healthz`는 Redis 연결까지 확인하며, compose의 `depends_on: condition: service_healthy`와 결합해 의존 서비스가 준비된 뒤에만 기동됩니다. 워커는 별도의 앱 레벨 재시도 루프를 두지 않고, 연결 장애 시 프로세스를 종료해 `restart: unless-stopped`(Docker)가 재기동하도록 복구를 위임합니다. 재기동 시 미ACK 메시지는 PEL에서 회수되므로 유실이 없고, 복구 책임이 오케스트레이터로 일원화되어 코드가 단순해집니다.

**유휴 스트림 블로킹 read 처리** — 워커는 `XREADGROUP ... BLOCK`으로 새 로그를 대기하는데, 로그가 없으면 소켓 read가 주기적으로 타임아웃됩니다(정상 동작). 이를 실제 장애와 구분해, 블로킹 만료(`redis.TimeoutError`)는 무시하고 재시도하며 진짜 연결 장애(`ConnectionError` 등)만 크래시→재시작으로 처리합니다.

**스트림 상한(`MAXLEN ~ 1,000,000`)** — 워커가 장기간 죽은 극단 상황에서 Redis 메모리가 무한히 팽창하는 것을 막는 최후의 안전판입니다. 근사(`~`) 트리밍으로 성능 손실 없이 동작합니다.
