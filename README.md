...
**수집과 적재를 분리**했습니다. API는 로그를 검증해서 Redis Stream에 `XADD`하고 즉시 응답합니다. 피크에는 로그가 큐에 쌓였다가 트래픽이 잦아들면 컨슈머가 처리하므로, MongoDB가 느려지거나 재시작 중이어도 수집은 멈추지 않습니다.
...
- **healthcheck 기동 순서** — `depends_on: service_healthy`로 Redis·Mongo 준비 후에만 API·워커 기동 
